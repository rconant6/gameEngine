const std = @import("std");
const Allocator = std.mem.Allocator;
const scene_format = @import("scene-format");
const SceneFile = scene_format.SceneFile;
const load = @import("loader.zig");
const debug = @import("debug");
const log = debug.log;

pub const SceneManager = struct {
    persistent: Allocator,
    io: std.Io,
    scenes: std.StringHashMap(*SceneFile),
    scene_file_paths: std.StringHashMap([]const u8),
    active_scene_name: ?[]const u8,

    pub fn init(persistent: Allocator, io: std.Io) SceneManager {
        return .{
            .persistent = persistent,
            .io = io,
            .scenes = .init(persistent),
            .scene_file_paths = .init(persistent),
            .active_scene_name = null,
        };
    }
    pub fn deinit(self: *SceneManager) void {
        log.info(.scene, "Scene Manager shutting down...", .{});
        var path_iter = self.scene_file_paths.iterator();
        while (path_iter.next()) |entry| {
            self.persistent.free(entry.value_ptr.*);
        }
        self.scene_file_paths.deinit();

        var iter = self.scenes.iterator();
        while (iter.next()) |entry| {
            self.persistent.free(entry.key_ptr.*);
            entry.value_ptr.*.deinit(self.persistent);
            self.persistent.destroy(entry.value_ptr.*);
        }
        self.scenes.deinit();

        if (self.active_scene_name) |as| self.persistent.free(as);
    }

    pub fn loadScene(
        self: *SceneManager,
        name: []const u8,
        file_path: []const u8,
    ) !void {
        if (self.scenes.contains(name)) {
            log.warn(.scene, "Scene {s} is already loaded", .{name});
            return SceneManagerError.SceneAlreadyLoaded;
        }

        const scene = load.loadSceneFile(self.persistent, self.io, file_path) catch |err| {
            log.err(.scene, "Unable to load {s}: {any}", .{ name, err });
            return err;
        };

        const owned_scene = try self.persistent.create(SceneFile);
        errdefer self.persistent.destroy(owned_scene);
        owned_scene.* = scene;

        const owned_name = try self.persistent.dupe(u8, name);
        errdefer self.persistent.free(owned_name);

        const owned_path = try self.persistent.dupe(u8, file_path);
        errdefer self.persistent.free(owned_path);

        try self.scenes.put(owned_name, owned_scene);
        try self.scene_file_paths.put(name, owned_path);
    }
    pub fn unloadScene(self: *SceneManager, name: []const u8) !void {
        if (self.active_scene_name) |as|
            if (std.mem.eql(u8, as, name)) {
                log.warn(.scene, "{s} is the active scene", .{name});
                return SceneManagerError.CannotUnloadActiveScene;
            };

        const scene_entry = self.scenes.fetchRemove(name) orelse {
            log.warn(.scene, "{s} cannot be unloaded", .{name});
            return SceneManagerError.SceneNotFound;
        };
        const path_entry = self.scene_file_paths.fetchRemove(name);

        scene_entry.value.deinit(self.persistent);
        self.persistent.destroy(scene_entry.value);
        self.persistent.free(scene_entry.key);

        if (path_entry) |pe| {
            self.persistent.free(pe.value);
        }
    }
    pub fn reloadActiveScene(self: *SceneManager) !void {
        const scene_name = self.active_scene_name orelse
            return SceneManagerError.NoActiveScene;
        const file_path = self.scene_file_paths.get(scene_name) orelse
            return SceneManagerError.SceneNotFound;

        const new_scene = try load.loadSceneFile(self.persistent, self.io, file_path);

        const old_scene = self.scenes.get(scene_name).?;
        old_scene.deinit(self.persistent);
        old_scene.* = new_scene;
    }

    pub fn setActiveScene(self: *SceneManager, name: []const u8) !void {
        if (!self.scenes.contains(name)) return SceneManagerError.SceneNotFound;
        if (self.active_scene_name) |old_name| self.persistent.free(old_name);
        self.active_scene_name = try self.persistent.dupe(u8, name);
    }
    pub fn getActiveScene(self: *SceneManager) ?*const SceneFile {
        if (self.active_scene_name) |active_scene|
            return self.scenes.get(active_scene);
        return null;
    }

    pub fn getScene(self: *SceneManager, name: []const u8) ?*const SceneFile {
        return self.scenes.get(name);
    }
};
pub const SceneManagerError = error{
    SceneAlreadyLoaded,
    SceneNotFound,
    CannotUnloadActiveScene,
    NoActiveScene,
};
